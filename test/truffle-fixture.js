...
const TestSmartContractWallet = artifacts.require(
  "./TestSmartContractWallet.sol"
);
const TestAuthenticatedProxy = artifacts.require(
...
module.exports = async (deployer, network) => {
  ...
  if (network !== "development") {
    setConfig(
      "deployed." + network + ".WyvernAtomicizer",
  ...
  if (network !== "development") {
    ...
    setConfig(
      "deployed." + network + ".TestAuthenticatedProxy",
      TestAuthenticatedProxy.address
    );
    setConfig("deployed." + network + ".TestERC1271", TestERC1271.address);
    setConfig(
      "deployed." + network + ".TestSmartContractWallet",
      TestSmartContractWallet.address
    );
  }
};
